...
public class TATransformer implements IClassTransformer {
    ...
    static {
        // required as there is no generic hardness hook for calls outside of player
        // breaking
        TRANSFORMERS.add(new TransformerWardBlockHardness());
        // required as there is no generic resistance hook for calls outside of player
        ...
        // the position is also not determined and put into the AI fields until the
        // event has already passed, so no reflection
        ...
        // required because TC calls Block#getBlockHardness instead of the blockstate
        // method
        TRANSFORMERS.add(new TransformerWardBlockTaintImmunity());

        // required to cancel sprinting client side immediately
        // using events allows 1-tick sprints, and holding down sprint will allow
        // constant sprinting
        ...
        // fixes annoying robe legging flapping (as if the player is walking) while
        // elytra flying
        ...
        // RenderWorldLastEvent almost works but it is after render pass 0, so it will
        // draw over transparent blocks
        TRANSFORMERS.add(new TransformerRenderEntities());

        // required to fix morphic tool recipe dupe while also allowing container items
        // to be used
        // backup non-coremod mitigation disables container items, which players
        // expressed a desire to keep
        TRANSFORMERS.add(new TransformerInfusionLeftoverItems());

        // required to have augments on baubles detected properly without having to loop
        // over them all every tick
        // performance with the looping method is terrible, and a change event should
        // really have been added...
        TRANSFORMERS.add(new TransformerBaubleSlotChanged());

        // required because TC always creates fog near eldritch guardians if not in the
        // outer lands
        ...
        // required because any attempt by TC to render a tile entity in a blueprint
        // that is
        ...
        // also the trajectory and target code is pretty much copy pasted with a
        ...
        // to allow wildcard metadata in required research items when they are
        // non-damageable
        TRANSFORMERS.add(new TransformerCycleItemStackMetadata());

        // to fix the forge bug that calls invalidate on tiles if they are loaded while
    ...
    public byte[] transform(String name, String transformedName, byte[] basicClass) {
        if (ThaumicAugmentationCore.isEnabled()) {
            ...
            for (ITransformer transformer : TRANSFORMERS) {
                if (transformer.isTransformationNeeded(node, transformedName)) {
                    if (ThaumicAugmentationCore.getExcludedTransformers().contains(transformer.getClass().getName())) {
                        ThaumicAugmentationCore.getLogger().info("Excluding transformer {} due to config request",
                                transformer.getClass().getName());
                    }
                    else if (!transformer.transform(node, transformedName)) {
                        ThaumicAugmentationCore.getLogger()
                                .error("A class transformer has failed! This is probably very bad...");
                        ThaumicAugmentationCore.getLogger()
                                .error("Class: " + transformedName + ", Transformer: " + transformer.getClass());
                        if (transformer.getRaisedException() != null) {
                            ThaumicAugmentationCore.getLogger().error("Additional information: ",
                                    transformer.getRaisedException());
            ...
            if (didSomething) {
                ClassWriter writer = !computeFrames ? new ClassWriter(ClassWriter.COMPUTE_MAXS)
