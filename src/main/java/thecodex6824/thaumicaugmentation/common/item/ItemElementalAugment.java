...
public class ItemElementalAugment extends ItemTABase implements IAugment, IAssociatedAspect {
    ...
    public boolean isCompatible(ItemStack stack, ItemStack otherAugment) {
        return !(otherAugment.getItem() instanceof ItemElementalAugment);
    }
    
    private void setPackagePower(FocusPackage p, float power) {
        try {
            PACKAGE_POWER.setFloat(p, p.getPower() * power);
        }
        catch (Exception ex) {
    ...
    public void onUserCast(ItemStack stack, ItemStack caster, FocusPackage focusPackage, Entity user) {
        ...
        setPackagePower(focusPackage, totalMultiplier);
    }
    ...
    @Override
    public Iterable<String> getAdditionalAugmentTooltip(ItemStack stack) {
        Aspect aspect = getAspect(stack);
        LinkedList<String> text = new LinkedList<>();
        text.add(new TextComponentTranslation("thaumicaugmentation.text.elemental_aspect", AspectUtil.getChatColorForAspect(aspect, DISALLOWED_COLORS) + aspect.getName()).getFormattedText());
        return text;
    }
